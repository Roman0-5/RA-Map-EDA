...
def create_annotation_table(df,
                            protein_id_col,
                            gene_symbol_col,
                            gene_name_col):
    """
    Creates annotation table with:
    ProteinID_GeneID + Gene Name
    """

    annotation = df[[protein_id_col, gene_symbol_col, gene_name_col]].copy()

    # create combined identifier
    annotation["ProteinID_GeneSymbol"] = (
        annotation[protein_id_col].astype(str) + "_" +
        annotation[gene_symbol_col].astype(str)
    )

    # keep only what you want
    annotation = annotation[["ProteinID_GeneSymbol", gene_name_col]]

    # clean
    annotation = annotation.drop_duplicates().dropna()

    annotation.columns = ["ProteinID_GeneSymbol", "Gene_Name"]
...
#---------------------------#
#SELECTION of PROTEIN data
#---------------------------#

def select_top_variance_features(df, top_n=50):
    """
    Select top N most variable numeric features.
    """

    # keep only numeric columns
    df_numeric = df.select_dtypes(include=["number"])

    variances = df_numeric.var(axis=0)
    top_features = variances.sort_values(ascending=False).head(top_n).index

    return df_numeric[top_features], variances.sort_values(ascending=False)

def map_selected_features_to_genes(selected_df, annotation_df):
    """
    Maps protein feature columns (variance-selected) to Gene Names.

    Parameters:
    - selected_df: dataset with protein columns (e.g., top variance features)
    - annotation_df: lookup table with ProteinID_GeneID + Gene_Name
    """

    # convert annotation table into lookup dictionary
    mapping = dict(
        zip(annotation_df["ProteinID_GeneSymbol"],
            annotation_df["Gene_Name"])
    )

    # map column names → gene names
    mapped_columns = [
        mapping.get(col, col)  # fallback = original name if missing
        for col in selected_df.columns
    ]

    # create renamed copy
    renamed_df = selected_df.copy()
    renamed_df.columns = mapped_columns
